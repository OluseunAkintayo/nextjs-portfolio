				...
				<div className='p-6 text-sm grid gap-6 grid-cols-1 sm:grid-cols-2 mt-32'>
					{
						projects.map((project) => (
							<div key={project.id} className="project flex flex-col gap-4 cursor-pointer px-8 py-10 rounded-lg transition-all border border-gray-700 hover:bg-gray-800/30">
								...
								<p className="text-left text-sm text-gray-400/90 leading-4">Made with {project.madeWith}</p>
								<div className='text-xs flex gap-4 mt-6'>
									{
										project.url &&
										<a href={project.url} target="_blank" rel="noreferrer" className="project-link text-gray-400/90 uppercase font-semibold tracking-widest">Visit</a>
									}
									{
										project.github &&
										<>
											| <a href={project.github} target="_blank" rel="noreferrer" className="project-link text-gray-400/90 uppercase font-semibold tracking-widest">View source code</a>
										</>
									}
								</div>
							</div>
